...
\frame{
	...
	\begin{itemize}[<+->]
		...
		\item Ciphertext indistinguishability (TODO)
	\end{itemize}
}
...
\frame{
	...
	\begin{itemize}[<+->]
		\item Allows computation on ciphertext
		\item Given function $f$ and $\textnormal{Enc}(x)$,
			compute $\textnormal{Enc}(f(x))$ without decrypting.
